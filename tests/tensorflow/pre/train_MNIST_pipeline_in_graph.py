...
test_dict = {
    ...
    nn.nn.placeholder_nodes["y_"]: test_labels}
...
train_nodes = nn.nn.get_list_of_nodes(["merged", "train_step", "accuracy", "global_step","loss"])
...
with open(params.output_file, "w") as of:
	...
	while continue_flag:
		...
		feed_dict = {
			...
			nn.nn.placeholder_nodes["learning_rate"]: FLAGS.step_width
		}
		...
		loss_eval, acc_eval, grad_eval = nn.sess.run([nn.loss, nn.nn.summary_nodes["accuracy"], gradients_t], feed_dict=test_dict)
