...
class TrajectoryJob_check_minima(TrajectoryJob):
    ...
    def run(self, _data):
        ...
        if len(_data.minimum_candidates) > self.MAX_CANDIDATES:
            # sort the gradients
            gradient_at_minima_candidates = [_data.gradients[i] for i in _data.minimum_candidates]
            ...
            assert( gradient_at_minima_candidates[ minima_indices[0] ] <= gradient_at_minima_candidates[ minima_indices[self.MAX_CANDIDATES] ] )
            if abs(gradient_at_minima_candidates[ minima_indices[0] ] - gradient_at_minima_candidates[ minima_indices[self.MAX_CANDIDATES] ]) < 1e-10:
                ...
                candidates = [_data.minimum_candidates[i] for i in np.random.choice(
                    ...
                    replace=False)]
                logging.info("Too many candidates with same lowest gradient, we look randomly at these: "+str(candidates))
            else:
                candidates = [_data.minimum_candidates[i] for i in minima_indices[0:self.MAX_CANDIDATES]]
                logging.info("Too many candidates, we look at ones with lowest gradient: "+str(candidates))
        else:
